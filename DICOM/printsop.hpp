...
class	AbstractPrintManagementMeta	:
	...
	public:
		...
		virtual	BOOL	GetPrinterInfo ( PDU_Service *thePDU,
		...
		//[add by peak
		BOOL SaveToDisk(AbstractImageBox& theImgBox,DICOMCommandObject	*theDCO);
		virtual		BOOL	uGetUID ( UID &uid ) = 0;
		//add]
...
//peak
class CPrintConfig:NGetRQ,NGetRSP
{
public:
	virtual BOOL GetUID(UID&uid);
	BOOL GetConfiguration( PDU_Service *pPDU,DICOMCommandObject*pDCO,DICOMDataObject *pDDO);
};
class CPrintQueue:NGetRQ,NGetRSP,NEventReportRQ,NEventReportRSP
{
public:
	virtual BOOL GetUID(UID&uid);
	BOOL GetQueueInfo(PDU_Service *pPDU,DICOMCommandObject*pDCO,DICOMDataObject *pDDO);
};
