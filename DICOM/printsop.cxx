...
//[add by peak
BasicGrayscalePrintManagementMeta::	uGetUID (
	UID		&theUID )
	{
	theUID.Set("1.2.840.10008.5.1.1.9");
	return ( TRUE );
	}
BasicColorPrintManagementMeta::	uGetUID (
	UID		&theUID )
	{
	theUID.Set("1.2.840.10008.5.1.1.18");
	return ( TRUE );
	}
//add]
BOOL
...
StandardFilm	::	WriteCreate (
	...
	if ( ! NCreateRSP :: Read ( &aDCO, thePDU, theDDO ) )
		...
		printf("NCreateRSP :: Read - DDO DUMP\n");
//		DumpDDO(theDDO);  //removed by Lin
...
//[add by peak
//you must give me sufficient space to hold the col	
BOOL ParseFilmSize(char*strFilmSize,int& Width,int&Height)
{
	if(strFilmSize[0]==0)
		return FALSE;
	if(!strnicmp(strFilmSize,"8INX10IN",8))
	{
		Width=203;
		Height=254;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"8_5INX11IN",10))
	{
		Width=216;
		Height=279;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"10INX12IN",9))
	{
		Width=254;
		Height=305;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"10INX14IN",9))
	{
		Width=257;
		Height=364;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"11INX14IN",9))
	{
		Width=279;
		Height=356;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"11INX17IN",9))
	{
		Width=279;
		Height=432;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"14INX14IN",9))
	{
		Width=356;
		Height=356;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"14INX17IN",9))
	{
		Width=356;
		Height=432;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"24CMX24CM",9))
	{
		Width=240;
		Height=240;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"24CMX30CM",9))
	{
		Width=240;
		Height=300;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"A4",2))
	{
		Width=210;
		Height=297;
		return TRUE;
	}
	if(!strnicmp(strFilmSize,"A3",2))
	{
		Width=297;
		Height=420;
		return TRUE;
...
BOOL ParseImageFormat(char*strImgFmt,char*strFilmSize,int& cols,int col[])
{
	int tmp;
	int Width,Height;
	Width=500;
	Height=500;
	if(!strnicmp(strImgFmt,"STANDARD",8))
	{
		sscanf(strImgFmt+9,"%d,%d",&cols,&tmp);
	...
	int Type=0;
	if(!strnicmp(strImgFmt,"ROW",3))
	{
		Type=1;
	}
	if(!strnicmp(strImgFmt,"COL",3))
	{
		Type=2;
	}
	if(Type==1||Type==2)
	{
		sscanf(strImgFmt,"%d",&(col[0]));
		cols=1;
		for(int j=1;;j++)
		{
			char*pstr=strchr(strImgFmt,',');
			if(pstr==NULL)
				break;
			sscanf(pstr+1,"%d",&(col[j]));
			cols++;
		}
		return TRUE;
	}
	if(ParseFilmSize(strFilmSize,Width,Height)==FALSE)
		return FALSE;
	if(!strnicmp(strImgFmt,"SLIDE",5))
	{
		cols=Width/35;
		tmp=Height/35;
		for(int i=0;i<cols;i++)
	...
	if(!strnicmp(strImgFmt,"SUPERSLIDE",10))
	{
		cols=Width/40;
		tmp=Height/40;
		for(int i=0;i<cols;i++)
...
//[add by peak
	VR* vr=GetVR(0x2010,0x0010);
	if(vr&&vr->Length>0)
	{
		char pstrFMT[20],pstrFilmSize[20];
		memset(pstrFMT,0,20);
		memset(pstrFilmSize,0,20);
		memcpy(pstrFMT,vr->Data,vr->Length);
		vr=GetVR(0x2010,0x0050);
		if(vr&&vr->Length>0)
		{
			memcpy(pstrFilmSize,vr->Data,vr->Length);
			pstrFilmSize[vr->Length]='\0';
		}	
		int col[200];
		int cols;
		ParseImageFormat(pstrFMT,pstrFilmSize,cols,col);
		for(int i=0;i<cols;i++)
		{
			for(int j=0;j<col[i];j++)
			{
				AbstractImageBox*pABOX=new AbstractImageBox(*itsManager);
				pABOX->ProtoGen();
				itsAbstractImageBoxs->Add(pABOX);
			}
		}
	}
//add]
...
AbstractFilmBox::SetImageBoxInstanceUIDS()
	...
	//add by peak
//	if(aVR->SQObjectArray==NULL)
//		PDU_Service::ParseRawVRToSQArray(aVR);
	if(aVR && aVR->SQObjectArray)
		{
...
/*
//edit by peak
BOOL AbstractFilmBox::SetImageBoxInstanceUIDS()
	{
...
/*	DICOMObject ddo;
	...
	{
		ddo.Push(vr);
		sprintf(tt,"%x,%x:",vr->Group,vr->Element);
	}
	while(vr=ddo.Pop())
		Push(vr);
*//*	if(aVR && aVR->SQObjectArray)
	...
	while((aSQArray==NULL||anIndex < aSQArray->GetSize()) &&
		anIndex < itsAbstractImageBoxs->GetSize())
	{
...
//[change by peak
	...
	char temp2[65];
	memset(temp,0,65);
	theSOPClassUID.GetBuffer(64);
	memcpy(temp,theSOPClassUID.GetBuffer(64),10);
	temp[10]='\0';
	NewSOPInstanceUID(temp,temp2);
	theUID.Set(temp2);
...
AbstractPrintManagementMeta	::	AbstractPrintManagementMeta()
	{
		itsAbstractPrinter = new AbstractPrinter(*this);
...
//[add by peak
BOOL AbstractPrintManagementMeta::SaveToDisk(AbstractImageBox& theImgBox,DICOMCommandObject	*theDCO)
{
	char*pFileName=tempnam("c:\\temp","DCMPRN");
	PDU_Service pdu;
	DICOMDataObject ddo;
	DICOMDataObject bkddo;
	VR* vr,*vr1;
	BYTE *data;
	while(vr=this->Pop())
	{
		...
		vr1=new VR(vr->Group,vr->Element,vr->Length,data);
		ddo.Push(vr1);
	}
	while(vr=bkddo.Pop())
	{
	...
	pdu.SaveDICOMDataObject(pFileName,DICOM_MEDIA_IMPLICITLITTLEENDIAN,&ddo);
	ddo.Reset();
	return TRUE;
}
//add]
...
SetUIDFromSQ(
	...
	UINT		Num = 0)
	{
	VR	*SQA = DDO->GetVR(SQGroup, SQElement);
	if ( ! SQA )
		return ( FALSE );

	Array < DICOMDataObject * >	*ADDO =
		(Array<DICOMDataObject*>*) SQA->SQObjectArray;
	...
	if ( Index > ADDO->GetSize() )
		return ( FALSE );
			
	DICOMDataObject	*vDDO = ADDO->Get(Index);
	
	VR	*ATA = vDDO->GetVR(ATGroup, ATElement);
...
//DEBUG:MARK
//		SystemDebug.printf("itsSOPInstanceUID = %s BEFORE\n", itsSOPInstanceUID->GetBuffer(1));
		
		// Extract the
		//SetUIDFromSQ(this, 0x2020, 0x0510, 0x0008, 0x1155, itsSOPInstanceUID);
	...
	while(aFilmBoxIndex < itsAbstractFilmBoxs->GetSize())
		...
		if ( !aFilmBox->IsCreated() )
			...
			// to the sequence
//DEBUG:MARK
//	SystemDebug.printf("itsSOPInstanceUID = '%s'\n", itsSOPInstanceUID->GetBuffer(1) );
...
//DEBUG
//	SystemDebug.printf("aFilmBox->itsSOPInstanceUID = %s\n", aFilmBox->itsSOPInstanceUID->GetBuffer(1));
...
//DEBUG
//	SystemDebug.printf("aImageBox->itsSOPInstanceUID = %s\n", aImageBox->itsSOPInstanceUID->GetBuffer(1));
					IdleProc (&itsLock);
...
AbstractPrintManagementMeta	::	PrintFilm( PDU_Service *thePDU,
	...
	if ( !Write( thePDU ) )
		{
//[peak
		MessageBox(NULL,"PrintFilm:Write","DICOMERROR",MB_OK);
//]
		return FALSE;
		}
	...
	if ( !aFilmBox )
		{
//[peak
		MessageBox(NULL,"PrintFilm:Get","DICOMERROR",MB_OK);
//]
		return FALSE;
		}
...
//[peak
		MessageBox(NULL,"PrintFilm:WritePrint","DICOMERROR",MB_OK);
//]
...
BOOL
AbstractPrintManagementMeta	::	GetPrinterInfo (
		PDU_Service *thePDU,
		DICOMDataObject *theDDO)
	{
...
BOOL
BasicColorPrintManagementMeta::ReadBox(
	PDU_Service *thePDU,
	DICOMCommandObject *theDCO,
	DICOMDataObject *theDDO)
	{
	return itsBasicColorImageBox.Read(thePDU, theDCO, theDDO);
	}

BOOL
BasicColorPrintManagementMeta::WriteBox(
	PDU_Service *thePDU,
	UID *theUID,
	DICOMDataObject *theDDO )
	{
	return itsBasicColorImageBox.Write(thePDU, theUID, theDDO, *this);
	}
