...
void CCustomItemColor::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* /*pButton*/)
{
	...
	pColorPopup->Create(rcItem, m_pGrid, CPS_XT_NOFILL|CPS_XT_MORECOLORS|CPS_XT_SHOW3DSELECTION|CPS_XT_SHOWHEXVALUE, GetColor(), GetColor());
	pColorPopup->SetOwner(m_pGrid);
	pColorPopup->SetFocus();
	pColorPopup->AddListener(pColorPopup->GetSafeHwnd());
	pColorPopup->m_pItem = this;*/

	CXTColorDialog* pColorDlg = new CXTColorDialog(GetColor(), GetColor(),
		( (CPS_XT_SHOWHEXVALUE | CPS_XT_SHOW3DSELECTION)), m_pGrid);
	...
	if (pColorDlg->DoModal() == IDOK)
	{
		...
		this->OnValueChanged(RGBToString((COLORREF)clrDlgColor));
	}
