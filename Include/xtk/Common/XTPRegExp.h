...
public:
	void SetPattern(LPCTSTR lpszPattern)
	{
		if (!CreateInstance())
			return;
		m_regexpPtr->put_Pattern(_bstr_t(lpszPattern));
	}
...
public:
	BOOL Test(LPCTSTR lpszSourceString)
	{
		...
		if (FAILED(m_regexpPtr->Test(_bstr_t(lpszSourceString), &bTest)))
			return FALSE;
	...
	BOOL Replace(LPCTSTR lpszSourceString, LPCTSTR lpszReplaceString, CString& strDestString)
	{
		...
		BSTR bstrResult = 0;
		if (FAILED(m_regexpPtr->Replace(_bstr_t(lpszSourceString), _bstr_t(lpszReplaceString), &bstrResult)))
			return FALSE;
	...
	long Execute(LPCTSTR lpszSourceString)
	{
		...
		IDispatch* _result = 0;
		if (SUCCEEDED(m_regexpPtr->Execute(_bstr_t(lpszSourceString), &_result)))
		{
