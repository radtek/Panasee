...
AFX_INLINE HRESULT VARCMP_S(LPVARIANT pvarLeft, LPVARIANT pvarRight, LCID lcid, ULONG dwFlags)
{
#if (_MSC_VER <= 1100) // Using Visual C++ 5.0
	HINSTANCE hModule = GetModuleHandle(_T("OLEAUT32.DLL"));
	if (!hModule)
		return E_FAIL;

	typedef HRESULT (STDAPICALLTYPE * PFNVARCMP)(LPVARIANT pvarLeft, LPVARIANT pvarRight, LCID lcid, ULONG dwFlags);
	PFNVARCMP pfnVarCmp = (PFNVARCMP)GetProcAddress(hModule, "VarCmp");
	if (!pfnVarCmp)
		return E_FAIL;

	return (*pfnVarCmp)(pvarLeft, pvarRight, lcid, dwFlags);
#else
