...
public:
	...
	//      uDataSizeLimit          : [in] The number of bytes to stop storing text to archive
	//                                     (or -1 to store all).
	//-----------------------------------------------------------------------
	...
	//      row1/col1 and row2/col2 (or lcStart and lcEnd) text coordinates.
	// Parameters:
...
AFX_INLINE BOOL CXTPSyntaxEditBufferManager::GetBuffer(const XTP_EDIT_LINECOL& lcStart,
					const XTP_EDIT_LINECOL& lcEnd, CMemFile& file, BOOL bColumnSelection,
