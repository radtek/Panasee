...
public:
	...
	//      strBufer - [out] Buffer for text to be created after expansion.
	...
	//      The display position.
	//-----------------------------------------------------------------------
	void SetCaretPos(CWnd* pWnd, const CSize& szSize, int nRow, int& rnCol,
					 BOOL bHideCaret = FALSE, BOOL bVirtualSpace = FALSE);

	void SetCaretByPoint(CWnd* pWnd, const CPoint& pt, const CSize& szSize,
						 int& rnRow, int& rnCol, BOOL bVirtualSpace = FALSE); //<COMBINE SetCaretPos@CWnd*@const CSize&@int@int&@BOOL@BOOL >
